...
class RegistroCliente extends Component {
    ...
    handlerSubmit = () => {
        ...
        fetch(`http://localhost:4000/usuario/add?nombre=${nombre}&correo=${correo}&telefono=${telefono}&password=${password}`)
            .then(response => {
                if (response.status === 200){
                    console.log('Usuario agregado')
                    this.props.history.push({pathname : ROUTE.URL_LOGIN_ALUMNO});
